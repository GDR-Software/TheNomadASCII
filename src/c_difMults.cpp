...
float SoloPlayer::M_spawnMultiplier()
{
...
float SoloPlayer::M_statMultiplier()
{
	game = std::make_unique<Game>();
	mob = std::make_unique<Mob>();
...
float SoloPlayer::P_statMultiplier()
{
	game = std::make_unique<Game>();
...
float SoloPlayer::I_spawnMultiplier()
{
	game = std::make_unique<Game>();
...
float SoloPlayer::I_statMultiplier()
{
	game = std::make_unique<Game>();
	if (game->dif == 0) {mult = game->i_mult - (noob_mult * extra_mult);}
	else if (game->dif == 1) {mult = game->i_mult - (recruit_mult * extra_mult);}
